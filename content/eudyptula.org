...
- configure automounting of the samba share[fn::In order to share files, we
  ...
  which you can read about [[id:db5b3b96-8ee6-4f86-8208-6958c1b28a34][here]].][fn::Yet another problem with the samba share
  I discuss [[id:062d9a4b-848d-4faf-8456-65d7546d08e9][here]].]
  #+begin_src bash
    mkdir -p /mnt/qemu
...
UAPI headers will be covered again in [[#task-15][task 15]].
*** Kbuild summarised
:PROPERTIES:
...
:END:
Punch it![fn::See [[id:062d9a4b-848d-4faf-8456-65d7546d08e9][appendix]] regarding an issue I encountered here.]
#+begin_src bash
  make modules
#+END_SRC
...
- =built-in.o= :: This file only applies to built-in modules, not loadable ones
  ...
  ~$(obj-y)~.  It later forms the =vmlinux= ELF binary.  ([[#task-02][Task 02]] will revisit
  this.)
*** Loading the module
:PROPERTIES:
...
[[https://git.kernel.org/][git.kernel.org]].  We'll talk about these a bit more in [[#task-07][task 07]], but for now,
...
For further notes on paring down a configuration file, see the [[id:db5b3b96-8ee6-4f86-8208-6958c1b28a34][appendices]].
...
static analysis in [[id:e91e9b02-d5b6-4120-b040-587ffafd9573][task 10]].)
...
demonstrated in [[id:4fafb3ff-d889-4385-8d19-5f19fe4038bc][task 01]]).
...
- =C={1,2}= :: This will perform static analysis on source files, as mentioned
  [[id:9b02819b-b51b-4339-8adc-99031847779a][above]].  =C=1= will check only those files that are [re]compiled; =C=2= will
  check all files.
...
bootloader entries.  (The =install= target is covered in more detail in [[id:97f80e94-2336-494f-a4f3-57a3f6c1d635][task
11]].)
...
I transition to [[id:670db2c8-3179-4003-9323-cd50b2b2a5bf][building and installing the kernel entirely within the VM]], as
...
- The short version :: The rules governing coding style in the Linux kernel
  ...
  =scripts/checkpatch.pl= (which will be employed in the [[id:bc6ba800-d10d-41c9-a9b3-646f4ea93c53][next task]]).  There is
...
More on this is covered later on in [[id:4cf49816-7559-408c-bea0-5f39131bba5d][task 10]], where we actually develop and
send a patch.
***** Feedback
:PROPERTIES:
...
module from [[id:5fd7fcd5-aa63-41f5-a706-0bbb1fb00b0a][task 01]], and (b) this other one I found:
#+name: coding_style.c
#+begin_src c
  #include <linux/module.h>
...
:END:
I mentioned the official coding style of the Linux kernel in

1 - [[id:503e1947-f374-4f8e-af59-7f30a9927256][task 03]]; [[id:f991e442-3bfb-40e5-a0d5-9252bfee2424][Patch submission overview]]

2 - [[503e1947-f374-4f8e-af59-7f30a9927256][task 03]];

3 - [[#89ff7eea-432e-4ee0-a17c-516b11107734][Task 03]]

[[https://www.kernel.org/doc/html/latest/process/coding-style.html][here's the link again]].  It's worth giving it read if you want to understand
...
:END:
[[id:503e1947-f374-4f8e-af59-7f30a9927256][As mentioned]], the =script/checkpatch.pl= script performs static analysis.  The
...
ignore those sections and [[id:88750d69-5d84-478c-9ea6-a4f5388fa3be][skip ahead]].
...
a =struct list_head= (more on that data structure [[id:459bf909-97d0-4e70-a002-e9c11ebca950][later]]), though its
...
(We will go over this topic more thoroughly in [[id:29787ffa-a04c-44ac-a030-7b1452bf710c][task 09]].)
...
on [[id:013c2fc3-cc14-4a15-918b-9044cef2102b][earlier]].  (In my experience, this didn't seem to process all consequents in
...
associated block device (more on major and minor numbers later in [[id:628566a0-dd52-425e-b686-b6e31e2ebdfe][task 06]]).
...
:END:
(Again, this is not what the task asked, so [[id:88750d69-5d84-478c-9ea6-a4f5388fa3be][skip ahead]] if you're not
...
handled by the =kmod= family of utilities[fn::See the [[id:b4a93119-0bd3-4bac-8275-68a40f8a6f69][appendix]] for more detail
...
- user-space (in a manner that requires further explication; see [[id:1b41b15a-5d3b-4eb9-a2f4-a2246de3533d][next
...
If you took the [[id:c94004f2-c0ce-490c-895c-1569717784b2][earlier detour]], you will have already learnt a way to test
...
:END:
As mentioned in my earlier [[id:c94004f2-c0ce-490c-895c-1569717784b2][detour]], when a uevent is sent from kernel to
...
module device table we covered [[id:88750d69-5d84-478c-9ea6-a4f5388fa3be][earlier]].  This information may then be collated
...
#+end_src
Huh?  This looks nothing like the alias that we [[id:88750d69-5d84-478c-9ea6-a4f5388fa3be][spotted earlier]] in the
...
=udevadm trigger= call I devised [[id:88750d69-5d84-478c-9ea6-a4f5388fa3be][earlier]] was not triggering the uevent for the
...
come across this again in [[id:253a35cd-3887-4b5c-a67b-b0dbdbed3cb0][task 11]].)  Presumably, the USB subsystem would then
...
in the [[id:b4a93119-0bd3-4bac-8275-68a40f8a6f69][appendices]] if you are interested in learning more.
* Task 06
:PROPERTIES:
...
can read in [[id:fb318868-a843-4e6c-8483-db73d832b726][LDD 3rd ed]].  This text was written in the 2.6.x era, before the
interface ([[id:0ead0429-3ed9-423d-ad8b-d38ab99e0df7][as I will describe]]) had existed.  After a cursory google, this
...
on this [[id:e174e541-7e7c-4077-956a-217cb3c4154d][later]].] directly, we must use the following:
#+BEGIN_SRC c
...
- From a glance of the error codes listed in =include/uapi/asm-generic/errno*=
  ...
  code is doing.  ([[id:d93ec70e-fc24-446e-a747-d9fde1bd70b2][Meaning is use]], as Wittgenstein would say.)  From what I
...
- Staging ::
  ...
  - as the name suggests, this directory is available for people to work on as
    yet incomplete drivers in public view (we'll encounter this again in [[id:4cf49816-7559-408c-bea0-5f39131bba5d][task
    10]])
...
pull=!  As I mentioned [[id:51d453ee-3820-493c-9a0b-7a1ef245fa14][above]], this tree doesn't work that way.  Always run =git
...
To boot this, we just repeat what we did in [[id:80b1401c-ac7d-41a0-af2a-4d29c6183058][task 02]].  Once there, we should
...
:END:
For the "id" file, we can just copy the code we used in [[id:1158c320-30bc-48d3-a42c-feea3eeb6e71][task 06]].  For the
...
page at module load time.  To allocate this page, [[id:fb318868-a843-4e6c-8483-db73d832b726][LDD]] suggests calling
...
this later in [[id:51ac94e1-515d-4e7f-a7ee-7a186091f63d][task 17]]).  If one occurred, we start over and ask the user to
retry:
#+begin_src c
...
- I was a bit unsure of how to respond if =copy_to_user= or =copy_from_user=
  ...
  copy takes place.
- I forgot to run =checkpatch.pl -f= on the module from [[id:1158c320-30bc-48d3-a42c-feea3eeb6e71][task 06]], so I did it
...
:END:
As I already touched on [[id:7dec8acc-8de6-4b4c-93d1-59850d71e7f7][earlier]], the structure of =/sys/= reflects the structure
...
- =/sys/devices/= :: The structure of this directory is a picture of the
  ...
  The value of =DEVPATH= --- as disclosed in a uevent (see [[id:c5e3bcd6-dd71-4efd-8f51-eb62ba994307][task 05]]) for a device
...
notable contents in the [[#procfs][appendix]].  (Of most similarity to sysfs might be the
...
[[id:775d8d65-72c1-4785-808c-472d4641d07e][strafe]] around to view things in a different set of words.
...
documented below.  The actual patch I submitted I will leave to [[id:a316f564-f9e6-450e-ad1f-8a1659a6f35c][the end]].
*** Email
:PROPERTIES:
...
- =git send-email= :: This generates and sends patch files.  Under the hood, it
  generates the patches with =git format-patch= (which we covered in [[id:89ff7eea-432e-4ee0-a17c-516b11107734][task 03]]).
...
=quilt= (a tool I mentioned earlier in [[id:51d453ee-3820-493c-9a0b-7a1ef245fa14][task 07]]) in lieu of =git am=.  The stable
...
may vary.  (If you want to see how I set up mine, check out the [[id:61c6e16b-a1bf-49ef-8bd9-4385461387b8][appendix]].)
...
specifying revisions and ranges]] earlier in [[id:89ff7eea-432e-4ee0-a17c-516b11107734][task 03]].)  But if we already have
...
:END:
We tested out the =checkpatch.pl= and =clang-format= tools [[id:bc6ba800-d10d-41c9-a9b3-646f4ea93c53][earlier]].
...
(I mention the actual patch /I/ submitted at [[id:a316f564-f9e6-450e-ad1f-8a1659a6f35c][the end]].)
...
We'll get to try this out in [[id:fde2b5a7-5542-42b5-a1f0-1bad9b560563][task 16]].
...
- =scatterlist/=
  - for testing scatterlists, which relate to Linux's [[https://en.wikipedia.org/wiki/Direct_memory_access][DMA]] layer (discussed in
    chapter 15 of [[id:fb318868-a843-4e6c-8483-db73d832b726][LDD]])
- =selftests/=
...
- [[https://syzkaller.appspot.com/upstream][Syzbot]]
  - see [[id:776e802a-d218-4a2a-8bda-4d37fd36d915][above]]
...
- =--norolestats= omits rolestats; statistics regarding potentially relevant
  ...
  =--cc-cmd= flags (see [[id:27563f63-557d-4f7b-afa4-39bd7107b4e7][below]]).
...
- includes the output of ~git log -1 --pretty=fixes <hash>~ of a commit that
  this patch fixes, if applicable (see my config from [[id:f94fde9c-9319-4be1-a04c-c065dc93d257][earlier]])
...
we'll get one final confirmation; see my config from [[id:58bd21c9-148d-49df-ad6d-ae92d1252601][earlier]].)
...
- booted the kernel up in the VM and loaded a copy of the module file as a
  ...
  - at this point I reconsidered whether to compile and install the kernel
    directly inside the VM instead (I bite the bullet on this in [[id:253a35cd-3887-4b5c-a67b-b0dbdbed3cb0][the following
    task]])
...
filesystem didn't have enough space for this.  See the [[id:51a7d5ac-e259-4d53-aa3d-27f4893f3a77][appendices]] for how I
remedied this.]
...
module.  I address this in the [[id:b32dc6ad-81bf-41fd-8c1d-24a72d6468f1][appendices]].
...
installed; I just ignored this.][fn::As mentioned in [[id:f7859da4-52ed-4327-a352-7c892bf99cd8][task 02]], we should always
...
- Interestingly, the device is registered with the input subsystem (and thus
  the driver lives in =drivers/input/=).  As to why I discuss [[id:d1a45dbc-5ca3-4b81-bc7b-bbe22295f591][later on]].
...
Likely it's related to the =snd-pcsp= module (see [[id:b32dc6ad-81bf-41fd-8c1d-24a72d6468f1][appendix]]).]) that this driver
...
- I made a few simple scripts for module-testing purposes, which I have
  ...
  install}.  I have given this in the [[id:11713263-20ae-4326-a076-dd7fcb4d0b76][appendices]].
- To previous implementations of this =id= file, I have made a couple of fixes.
  ...
  mentioned [[id:ddfd869d-eaa0-492d-85fb-e2827a3ec726][here]]).
...
:END:
The texts mentioned in the [[id:fb318868-a843-4e6c-8483-db73d832b726][introduction]] provide very helpful explanations on
...
:END:
As for [[id:3155bc29-4d2c-4b83-958b-c29008e1beee][task 12]], I'll just mention the most salient details here.
...
A brief overview of =/proc/= can be seen in the [[id:ad4353ca-42df-48b3-98ac-e9ebbf309dab][appendix]].  The =/proc/${PID}/=
...
Ooh, this looks like fun!  I'm going to start with chapter 5 of [[id:fb318868-a843-4e6c-8483-db73d832b726][LKD]]; hopefully
...
:END:
I mentioned sparse briefly back in [[id:e91e9b02-d5b6-4120-b040-587ffafd9573][task 10]].  The primary sources of
...
directory as the make target.  (As I noted earlier in [[id:e91e9b02-d5b6-4120-b040-587ffafd9573][task 10]], we /do not/
...
archives I found most useful, which you can see in the [[id:5bf573bb-0efb-4e59-b714-df47a537a1ea][appendices]].
* Task 17
:PROPERTIES:
...
process softirqs (and by extension tasklets; read [[id:fb318868-a843-4e6c-8483-db73d832b726][LKD]] to learn all about this
stuff).
...
- In =identity_create()=, I switched from =strncpy()= to the safer,
  null-terminating =strscpy()=.  (See my notes at end of [[id:de624811-3fef-4e50-86ce-73058ece10be][task 12]].)
...
- I /still/ have no idea what the =busy= flag in =struct identity= (introduced in
  [[id:3155bc29-4d2c-4b83-958b-c29008e1beee][task 12]]) was for.
...
- Regarding =mutex_lock_interruptible()=, =wait_event_interruptible()=, and
  ...
  [[id:1fa85feb-1770-490d-a705-2af56f13e57a][earlier discussion in task 17]].
...
[[id:bfbf5793-b0db-4708-bf7e-34ff8fcd1424][MUA]] may not involve the network stack at all.  That is, if the email is
...
multiple buffers, [[id:3f8d9bca-747a-4107-9011-a75747c533ed][as already discussed]].
...
- I was printing out packet data to the console in printable ascii and noticed
  that many of the packets were samba packets.  (Recall that I [[id:32aca210-6455-4ad6-9f83-78aba01cb38a][set up a samba
  share]] in my VM.)  I noticed this because the SMB headers contain a 4-byte
  string: ={ '\xFF', 'S', 'M', 'B' }=.
- As stated [[id:5574be25-c38b-4242-9017-f3e3fb9483c7][above]], the textsearch API depends on =CONFIG_TEXTSEARCH=.  I forgot
...
- Normally, a driver need not be concerned with checking for file permissions,
  ...
  though.  Chapter 6 of [[id:fb318868-a843-4e6c-8483-db73d832b726][LDD]] goes into this in more detail.[fn::ff.
...
:CUSTOM_ID: paring-down-a-kernel-config
:END:
...
:CUSTOM_ID: troubleshooting-cifs
:END:
...
:CUSTOM_ID: the-pc-speaker-device
:END:
...
covered in [[id:1b41b15a-5d3b-4eb9-a2f4-a2246de3533d][task 05]].].  But when we compile our own kernel, with both modules
...
:CUSTOM_ID: source-kernel-uninstall-script
:END:
#+begin_src bash
  #!/usr/bin/env bash
...
- =/proc/kcore= is a pseudo-file which represents a coredump of the running
  ...
  from an unstripped =vmlinux= file (see my notes on [[id:9b02819b-b51b-4339-8adc-99031847779a][the build process]]).
...
:CUSTOM_ID: setting-up-a-credential-helper-for-git
:END:
Following on from [[id:10772f62-bbcb-430a-8dae-e4e3d9902419][here]]: I use [[https://wiki.gnome.org/Projects/Libsecret][libsecret]] + [[https://wiki.gnome.org/action/show/Projects/GnomeKeyring][Gnome Keyring daemon]] as my
...
:CUSTOM_ID: mailing-list-archives
:END:
